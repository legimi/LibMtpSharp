...
namespace LibMtpSharpStandardMacOS.NativeAPI
{
    ...
    internal partial class LibMtpLibrary
    {
        ...
        [DllImport(LibMtpName, CallingConvention = CallingConvention.Cdecl)]
        private static extern int LIBMTP_Set_Friendlyname(IntPtr mtpDeviceStructPointer, byte[] friendlyName);

        public static int SetFriendlyName(IntPtr mtpDeviceStructPointer, string friendlyName) =>
            LIBMTP_Set_Friendlyname(mtpDeviceStructPointer, Encoding.UTF8.GetBytes(friendlyName));
